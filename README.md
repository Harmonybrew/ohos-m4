...
本项目为 OpenHarmony 平台编译了 m4，并发布预构建包。

本项目是 Harmonybrew 项目基础设施的一部分，主要供内部使用。我们也将其开源，有需要者可自取。

## 获取预构建包

前往 [release 页面](https://github.com/Harmonybrew/ohos-m4/releases) 获取。
...
export PATH=~/m4-1.4.20-ohos-arm64/bin:$PATH
...
cd /data
tar -zxf m4-1.4.20-ohos-arm64.tar.gz
export PATH=/data/m4-1.4.20-ohos-arm64/bin:$PATH
...
这个项目使用本地编译（native compilation，也可以叫本机编译或原生编译）的做法来编译鸿蒙版 m4，而不是交叉编译。

需要在 [鸿蒙容器](https://github.com/hqzing/dockerharmony) 中运行项目里的 build.sh，以实现 m4 的本地编译。

示例：
```sh
git clone https://github.com/Harmonybrew/ohos-m4.git
cd ohos-m4
docker run \
  --rm \
  -it \
